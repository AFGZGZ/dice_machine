...
function runDiceMachine() {
  ...
  /*Table Update all the other %*/
  for (var i = 1; i <= 6; i++) {
    if (i !== randomNumber) {
      document.getElementById("tDice" + i + "%").innerText = getOtherPercent(i);
    } else {
      //do nothing
    }
  }
...
function getOtherPercent(num) {
  if (num.innerText !== 0) {
    return (statsObject[num] * 100) / numberOfRolls;
  }
}
